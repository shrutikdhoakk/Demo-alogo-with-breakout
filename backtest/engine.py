...
class BacktestEngine:
    def __init__(
        ...
        # --- Build a read-only cfg view from StrategyConfig defaults (no instantiation) + YAML overlays ---
        base: Dict[str, Any] = {}
        try:
            for f in fields(StrategyConfig):  # read field defaults without creating the object
        ...
        for k in ("breakout_atr_buf", "trail_atr_mult", "atr_pct_max"):
            if k in self.cfg and self.cfg[k] is not None:
                try:
                    base[k] = float(self.cfg[k])
                except Exception:
                    base[k] = self.cfg[k]

        sc_view = _SNS(**base)
        print(
            "USING_PARAMS", "scope=engine->strategycfg",
